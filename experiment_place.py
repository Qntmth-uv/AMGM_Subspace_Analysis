...
codes_word = parent_dir.find('code')
import_directory = os.path.join(parent_dir, "minimal_stuff")

# Experiment parameters (Standar ones)
...
# Set it seed (randomtly generated)
...
def main():
    ...
    #Run the unitary experiments  
    #cProfile.run(f"unitary_test(dimensions, test_condition_numbers, args.dist, args.sys, seeds[:args.nexp], bool(args.info), args.iters, args.savedir, args.tol)", parent_dir+"cprof_"+exc_date_str+".prof")
  
    #Run the unitary experiments wo cprofile
    #unitary_test(dimensions_exponential, condition_numbers_exponential, args.dist, args.sys, seeds[:args.nexp], bool(args.info), args.iters, args.savedir, args.tol)

    #Run a set of experiments wo cprofile
